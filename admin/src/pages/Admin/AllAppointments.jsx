...
const AllAppointments = () => {
  ...
  const { aToken, appointments, cancelAppointment, getAllAppointments, clearCompletedAppointments } = useContext(AdminContext)
  const { slotDateFormat, calculateAge, currency } = useContext(AppContext)

  // Count completed/cancelled appointments
  const completedCount = appointments.filter(item => item.cancelled || item.isCompleted).length
    ...
    <div className='w-full max-w-6xl m-5 '>
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        {completedCount > 0 && (
          <button 
            onClick={() => {
              if (window.confirm(`Clear ${completedCount} completed/cancelled appointments?`)) {
